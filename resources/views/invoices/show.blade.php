                ...
                <div class="p-6 bg-white border-b border-gray-200">
                    ...
                    <div class="mb-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mt-2">
                            ...
                            <p><strong>Submit Date:</strong> {{ $invoice->submit_date ? $invoice->submit_date->format('d M Y H:i') : '-' }}</p>
                            {{-- Jika ada user_id --}}
                    ...
                    <table class="min-w-full divide-y divide-gray-200 mb-4">
                        ...
                        <tfoot>
                            <tr>
                                ...
                                <td class="px-4 py-2 text-right font-bold text-sm">{{ number_format($invoice->total_price ?? $invoice->total_amount ?? 0, 2, ',', '.') }}</td>
                            </tr>
                        </tfoot>
                    </table>

                    {{-- TEMPATKAN KODE TOMBOL DI SINI --}}
                    <div class="mt-6 flex justify-end space-x-2">
                        <a href="{{ route('invoices.print', $invoice->id) }}" target="_blank" class="bg-green-500 hover:bg-green-700 text-white font-bold py-2 px-4 rounded no-print">
                            Print Invoice
                        </a>
                        <a href="{{ route('invoices.index') }}" class="bg-gray-500 hover:bg-gray-700 text-white font-bold py-2 px-4 rounded no-print">
                            Back to List
                        </a>
                    </div>
                    {{-- AKHIR KODE TOMBOL --}}
